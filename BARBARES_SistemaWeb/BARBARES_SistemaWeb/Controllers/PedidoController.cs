...
namespace BARBARES_SistemaWeb.Controllers
{
    public class PedidoController : Controller
    {
        ...
        public ActionResult List_Parametros()
        {
            ...
            webrequest = (HttpWebRequest)WebRequest.Create(Constantes.SelectAll_TipoTarjeta);
            webrequest.Method = Constantes.PostMethod;
            webrequest.ContentType = Constantes.ContentType;
            webrequest.ContentLength = data.Length;
            ...
            using (var reader = new StreamReader(webresponse.GetResponseStream()))
            {
                JavaScriptSerializer js = new JavaScriptSerializer();
                var objText = reader.ReadToEnd();
                ViewBag.TipoTarjeta = (List<TipoTarjeta>)js.Deserialize(objText, typeof(List<TipoTarjeta>));
            }             

            //Select Moneda
            data = encoding.GetBytes("");

            webrequest = (HttpWebRequest)WebRequest.Create(Constantes.SelectAll_Moneda);
            webrequest.Method = Constantes.PostMethod;
            webrequest.ContentType = Constantes.ContentType;
            webrequest.ContentLength = data.Length;
